...
export default function SupportItemSelector(props) {
  ...
  useEffect(() => {
    if (loggedIn) {
      ...
      const supportCategory = JSON.parse(localStorage.getItem(PLAN_CATEGORIES));
      if (
  ...
  useEffect(() => {
    if (!loggedIn) {
      const supportCategory = JSON.parse(localStorage.getItem(PLAN_CATEGORIES))
        ? JSON.parse(window.localStorage.getItem(PLAN_CATEGORIES))
        : {};

      supportCategory[supportCategoryID] = planItems;

      localStorage.setItem(PLAN_CATEGORIES, JSON.stringify(supportCategory));
    }
  });
